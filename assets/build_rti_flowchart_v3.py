...
# Columns (center x)
C1, C2, C3, C4, AND_X, C5 = 360, 900, 1360, 1780, 2080, 2400
JUN = 630

# Large nodes, short stubs between them
BOX_W, BOX_H = 310, 92
PILL_W, PILL_H = 186, 70
START_W, START_H = 560, 110
AND_R = 36
DOT_R = 17
H_BOX = BOX_H / 2
H_PILL = PILL_H / 2
...
class Diagram:
    ...
    def time_pill(self, x, y, days, cite=""):
        ...
        self.nodes.append(
            ...
            <g class="node" transform="translate({x},{y})">
              ...
              <text y="8" text-anchor="middle" fill="{WHITE}">
                <tspan font-size="26" font-weight="700">{days}</tspan>
                <tspan font-size="14" font-weight="700" dx="8" letter-spacing="0.12em">DAYS</tspan>
              </text>
            </g>'''
        )
        if cite:
            self.caption(x + w / 2 + 14, y + 6, cite, anchor="start", size=16, fill=INK_FAINT, weight=650, tracking="0.04em")
    ...
    def box(self, x, y, text, kind="process", w=BOX_W, h=BOX_H):
        ...
        rx = 16 if kind == "start" else 14
        if kind == "start":
            w, h = START_W, START_H
        stroke_attr = f' stroke="{stroke}" stroke-width="{sw}"' if sw else ""
        lines = text if isinstance(text, (list, tuple)) else [text]
        n = len(lines)
        tracking = "0.08em" if kind == "start" else "0.04em"
        size = 28 if kind == "start" else 22
        if kind == "start" and n == 2:
            text_el = (
                f'<text y="-10" text-anchor="middle" fill="{tfill}" font-size="28" font-weight="650" letter-spacing="0.10em">{esc(lines[0])}</text>'
                f'<text y="22" text-anchor="middle" fill="{tfill}" opacity="0.84" font-size="16" font-weight="600" letter-spacing="0.08em">{esc(lines[1])}</text>'
            )
        else:
            title_y = 8 if n == 1 else -6
            tspans = []
    ...
    def and_join(self, x, y):
        self.nodes.append(
            ...
            <g class="node" transform="translate({x},{y})">
              ...
              <text y="6" text-anchor="middle" fill="{WHITE}" font-size="15" font-weight="700" letter-spacing="0.12em">AND</text>
            </g>'''
        )
  ...
  <defs>
    ...
    <marker id="arrow" viewBox="0 0 16 16" refX="14" refY="8" markerWidth="15" markerHeight="15" orient="auto" markerUnits="userSpaceOnUse">
      <path d="M 1 2 L 15 8 L 1 14 z" fill="{LINE_STRONG}"/>
    </marker>
  </defs>
  {''.join(band_svg)}
...
def build() -> Diagram:
    ...
    # Phase bands
    d.band(0, 470, BAND1, "01  PIO")
    d.band(470, 410, BAND2, "02  APPLICANT")
    d.band(880, 250, BAND3, "03  FAA")
    d.band(1130, 390, BAND4, "04  CIC / SIC")
    ...
    d.box(C5, y_t3, "NO TIME LIMIT", kind="limit", w=280)

    d.box(C1, y_sat1, "SATISFIED", kind="success")
    d.box(C3, y_app, "FIRST APPEAL", kind="appeal", w=380)
    d.box(C5, y_app, ["SECTION 18", "COMPLAINT TO CIC"], kind="alt", w=360, h=108)
    ...
    d.box(C3, y_sec, "SECOND APPEAL TO CIC / SIC", kind="appeal", w=680)
    d.caption(C3, y_sec + 58, "Within 90 days of the FAA decision or default", size=15, fill=INK_FAINT, weight=500)

    # --- Labels ---
    d.caption(C1, y_t1 - 46, "A  ·  REPLY RECEIVED", size=15, fill=NAVY, weight=700, tracking="0.10em")
    d.caption(C3, y_t1 - 46, "B  ·  TRANSFERRED", size=15, fill=NAVY, weight=700, tracking="0.10em")
    d.caption(C5, y_t1 - 46, "C  ·  NO REPLY", size=15, fill=NAVY, weight=700, tracking="0.10em")

    d.caption(C1 - BOX_W / 2 - 14, (y_p1 + y_sat1) / 2, "If satisfied", size=15, fill=GREEN, weight=650, anchor="end")
    d.caption(GUTTER - 12, y_not - 70, "If not satisfied", size=15, fill=RED, weight=650, anchor="end")
    d.caption(AND_X, y_p2 - 50, "Either no-reply path", size=15, fill=INK_FAINT, weight=650, tracking="0.04em")
    d.caption(C5, y_t3 - 48, "Parallel remedy", size=15, fill=SAFFRON, weight=700, tracking="0.08em")
    d.caption(JUN - 90, y_bot - 44, "If satisfied", size=15, fill=GREEN, weight=650)
    d.caption(JUN + 100, y_bot - 44, "If not satisfied", size=15, fill=RED, weight=650)

    # --- Wires ---
    # Start splits into three clocks
    rail_y = y_start + H_START + 12
    ...
    # Direct reply: short drop to Satisfied
    d.wire([(C1, y_p1 + H_BOX), (C1, y_sat1 - H_BOX)])

    # Direct reply: side path to Not Satisfied (gutter, avoids the long spine)
    d.wire([(C1 + BOX_W / 2, y_p1), (GUTTER, y_p1), (GUTTER, y_not), (C2 - BOX_W / 2, y_not)])

    # Transfer reply down to Not Satisfied
    d.wire([(C2, y_p2 + H_BOX), (C2, y_not - H_BOX)])

    # No-reply paths join at AND (no sideways arrowheads)
    d.wire([(C4 + BOX_W / 2, y_p2), (AND_X - AND_R, y_p2)], arrow=False)
    d.wire([(C5, y_p1 + H_BOX), (C5, y_p2), (AND_X + AND_R, y_p2)], arrow=False)

    asplit = y_p2 + AND_R + 12
    ...
    # Two §19(1) clocks merge into First Appeal on a shared bus
    merge = y_t3 + H_PILL + 12
    ...
    d.wire([(C5, y_t3 + H_BOX), (C5, y_app - 54)])

    # First appeal splits into 45-day clocks
    fsplit = y_app + H_BOX + 12
    ...
    # Decision forks at a junction: satisfied or not satisfied
    elbow = y_bot - H_BOX - 18
    ...
    # No decision down to its 90-day clock
    d.wire([(C4, y_dec + H_BOX), (C4, y_90 - H_PILL)])

    # Bottom not-satisfied down to its 90-day clock
    d.wire([(C2, y_bot + H_BOX), (C2, y_90 - H_PILL)])

    # Both 90-day clocks merge into Second Appeal on a shared bus
    sec_x = C3
    merge90 = y_90 + H_PILL + 12
