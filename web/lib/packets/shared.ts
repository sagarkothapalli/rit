...
export function createIndexPdf(record: CaseRecord, rules: FilingRuleSet): Blob {
  ...
  y += 6;
  const live = record.attachments.filter((item) => !item.deletedAt);
  live.forEach((item, index) => {
    y = paragraph(
      doc,
      `${index + 1}. ${item.storedName} — ${item.kind.replaceAll("_", " ").toLowerCase()} (${Math.ceil(item.byteSize / 1024)} KB)`,
      y,
      page,
      "Document index",
    );
